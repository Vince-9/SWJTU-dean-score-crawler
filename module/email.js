...
exports.sendMailNewGrade = function (email, grades, mode) {
	...
	if (/职场|英语|学术|视听说|高级|营销/ig.test(grades.className)) {
		...
		mailOptions = {
			...
			html: `<h1>Welcome to Vincent's email notification system</h1><h2>Tomorrow and grades, you never know which comes first.</h2><p>subject:</p><p>${grades.className}</p><p>final grades:<b>${grades.finalGrade}</b></p><p>paper grades：<b>${grades.paperGrade}</b></p><p>regular grades:<b>${grades.regularGrade}</b></p>`
			//验证码：${Math.ceil(Math.random() * 10000)} 这个验证码是为了反垃圾邮件的，并没有什么用`
		};
	} else {
		mailOptions = {
